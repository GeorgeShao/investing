...
/**
 * Monthly TWRR for one sub-period.
 * r_t = (V_t − F_t) / V_{t-1} − 1 when V_{t-1} > 0; null otherwise.
 * Treats external net flow as end-of-period for a simple monthly approximation.
 */
export function monthlyTwrr(
  ...
  if (!(priorNetWorth > 0)) return null;
  const f = externalNetFlow(deposits, withdrawals);
  return (netWorth - f) / priorNetWorth - 1;
}
